...
namespace Api.Controllers.Admin
{
    ...
    public class ProductsController : Controller
    {
        ...
        private IEnumerable<string> CreateLinkHeader(PagedList<Product> pagedList)
        {
            ...
            {
                var linkHeader = "<" + _urlHelper.Link("GetAdminProducts", new { pageNumber = pagedList.CurrentPage + 1, pageSize = pagedList.PageSize }).ToLowerInvariant() + ">; rel=next";
                linkHeaders.Add(linkHeader);
            }

            if (!pagedList.IsLastPage)
            {
                var linkHeader = "<" + _urlHelper.Link("GetAdminProducts", new { pageNumber = pagedList.TotalPages, pageSize = pagedList.PageSize }).ToLowerInvariant() + ">; rel=last";
                linkHeaders.Add(linkHeader);
            }

            if (!pagedList.IsFirstPage)
            {
                var linkHeader = "<" + _urlHelper.Link("GetAdminProducts", new { pageNumber = 1, pageSize = pagedList.PageSize }).ToLowerInvariant() + ">; rel=first";
                linkHeaders.Add(linkHeader);
            }

            if (pagedList.HasPrevious)
            {
                var linkHeader = "<" + _urlHelper.Link("GetAdminProducts", new { pageNumber = pagedList.CurrentPage - 1, pageSize = pagedList.PageSize }).ToLowerInvariant() + ">; rel=prev";
                linkHeaders.Add(linkHeader);
            }
